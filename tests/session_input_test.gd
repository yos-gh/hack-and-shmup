...
func run() -> void:
	...
	game.boss.salvos.append({"owner":game.enemies[0],"delay":1.0})
	game.boss.options.append({"owner":game.enemies[0],"life":1.0})
	game.boss.lasers.append({"owner":game.enemies[0],"duration":1.0})
	game.bullets.append({"hostile":true})
	game.return_to_title()
	check(game.boss.salvos.is_empty() and game.boss.options.is_empty() and game.boss.lasers.is_empty() and game.bullets.is_empty(),"leaving combat clears queued attacks and projectiles immediately")
	key(game,KEY_B)
	key(game,KEY_3)
	key(game,KEY_UP)
	key(game,KEY_ENTER)
