...
func run() -> void:
	...
	check(sound.hit_gap == 0.0, "blocked and lethal hits do not add nonlethal sound")
	game.combat_events.enemy_hit.emit(Vector2.ZERO, 1.0, false, false)
	game.combat_events.enemy_hit.emit(Vector2.ZERO, 1.0, false, false)
	check(sound.voices.filter(func(v): return v.playing and v.stream == sound.clips.hit).size() == 1, "simultaneous nonlethal hits coalesce")
