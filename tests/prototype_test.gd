...
func run() -> void:
	...
	check(game.sound.clips.size()==17 and ["warning","siege_fire","hunter_lock","hunter_fire","sniper_fire","halo_fire","halo_option","hunter_burst"].all(func(key): return game.sound.clips.has(key)) and game.sound.music.stream.loop_end > 0, "audio assets and loop loaded")
