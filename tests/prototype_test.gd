...
func run() -> void:
	...
	check(game.sound.clips.size()==13 and ["warning","siege_fire","hunter_lock","hunter_fire"].all(func(key): return game.sound.clips.has(key)) and game.sound.music.stream.loop_end > 0, "audio assets and loop loaded")
