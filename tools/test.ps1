...
foreach ($test in Get-ChildItem -LiteralPath (Join-Path $projectPath 'tests') -Filter '*_test.gd' | Sort-Object Name) {
    ...
    if ($test.Name -notin @('audio_test.gd', 'view_state_test.gd', 'depth_view_test.gd', 'background_view_test.gd', 'background_art_test.gd', 'background_edges_test.gd', 'camera_view_test.gd', 'sniper_warning_test.gd', 'hud_audio_test.gd', 'halo_view_test.gd', 'boss_art_test.gd', 'enemy_audio_test.gd')) { $arguments += '--headless' }
