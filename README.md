...
HALO ENGINEは本体の周囲145pxに3基の発射オプションを展開。攻撃ごとに三角形の向きを変え、予兆の後に時間差で155px/秒の3〜5-wayを発射します。連射・放射の段階では発射時に狙い、挟撃の段階では展開時の位置を狙うため、異なる場所からの交差射撃を誘導できます。オプションにはHP・接触判定・自機弾を遮る判定がなく、発射まで位置を固定。近距離に展開した際は猶予を0.35秒追加します。討伐・リトライ時に消去します。

SIEGE ARRAYは楕円上の6等分配置を基本に、角度4通りと横幅2通りの8配置から選びます。全配置で砲台間190px以上・入口から140px以上を確保し、数・HP・攻撃間隔は共通。同じ戦闘のリトライでは配置を維持、新しい戦闘では抽選し直します。
...
tests/boss_practice_test.gdで8配置の網羅と間隔、オプションからの発射・非衝突、キーボード／マウス選択、自動強化、リトライ、記録の分離を検証します。
