...
砲台は動かず、直進弾（橙）と最初の1秒だけ緩く追尾する弾（紫）を交互に発射。砲台が減るほど発射間隔が短くなり、半数を破壊すると3方向弾になります。残存砲台数と総HPを上部に表示します。各砲台のHPは現在のマシンガン火力を基準に約1.6秒分（最低8）。回避と移動を含めて20〜30秒程度を目安にした暫定値です。無敵の検証用移動・射撃スクリプトでは約11秒で撃破し、回避に使う余地を確保しています。
