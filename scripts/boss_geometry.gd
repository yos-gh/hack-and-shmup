...
const SIZE_SCALE := [1.5, 2.0, 2.0, 1.0, 1.0]
const AUTHORED_SCALE := [1.3, 1.12, 1.0, 1.0, 1.0]
...
static func hit_radius(variant: int) -> float:
	...
	var radius: float = [SIEGE_EXTENT, HUNTER_EXTENT.length(), HALO_RADIUS, 48.0, 48.0][variant]
