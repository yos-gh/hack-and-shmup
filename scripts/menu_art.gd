...
func _draw() -> void:
	...
	if kind == "title" or kind == "practice":
		...
		if kind == "title":
			var center := Vector2(size.x*0.5,size.y*0.5-190)
			for i in range(3):
			...
			preload("res://scripts/visual_icons.gd").draw_wordmark(self,Vector2(size.x*0.5,size.y*0.5-96),minf(580,size.x-120))
