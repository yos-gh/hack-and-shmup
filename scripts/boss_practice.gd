...
func button(screen: Vector2, index: int) -> Rect2:
	...
	var width := minf(240,(screen.x-120)/4)
	if index == 7: return Rect2(Vector2((screen.x-width*4-72)*0.5+3*(width+24),center.y-110),Vector2(width,72))
	if index < 3: return Rect2(Vector2((screen.x-width*4-72)*0.5+index*(width+24),center.y-110),Vector2(width,72))
...
func input(game, event: InputEvent) -> void:
	if event is InputEventKey and event.pressed and not event.echo:
		...
		elif event.is_action_pressed("practice_left"): variant = (variant+3)%4
		elif event.is_action_pressed("practice_right"): variant = (variant+1)%4
